...
# osin_hash
RUN git clone https://github.com/deosin/osin_hash
RUN cd osin_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} osin
RUN useradd -u ${USER_ID} -g osin -s /bin/bash -m -d /osin osin
...
RUN mkdir /osin-src && \
  ...
  chown $USER_ID:$GROUP_ID /osin-src && \
