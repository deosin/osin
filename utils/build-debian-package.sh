...
PACKAGE_VERSION=$($SRC_PATH/src/osind --version | grep version | cut -d' ' -f4 | tr -d v)
...
# Copy binaries
cp $SRC_PATH/src/osind $DEB_BIN
cp $SRC_PATH/src/osin-cli $DEB_BIN
cp $SRC_PATH/util/fetch-params.sh $DEB_BIN/osin-fetch-params
# Copy docs
...
# Copy manpages
cp $SRC_DOC/man/osind.1 $DEB_MAN
cp $SRC_DOC/man/osin-cli.1 $DEB_MAN
cp $SRC_DOC/man/osin-fetch-params.1 $DEB_MAN
# Copy bash completion files
cp $SRC_PATH/contrib/osind.bash-completion $DEB_CMP/osind
cp $SRC_PATH/contrib/osin-cli.bash-completion $DEB_CMP/osin-cli
# Gzip files
gzip --best -n $DEB_DOC/changelog
gzip --best -n $DEB_DOC/changelog.Debian
gzip --best -n $DEB_MAN/osind.1
gzip --best -n $DEB_MAN/osin-cli.1
gzip --best -n $DEB_MAN/osin-fetch-params.1

cd $SRC_PATH/contrib

# Create the control file
dpkg-shlibdeps $DEB_BIN/osind $DEB_BIN/osin-cli
