...
if [ "$uname_S" = "Darwin" ]; then
    PARAMS_DIR="$HOME/Library/Application Support/OsinParams"
else
    PARAMS_DIR="$HOME/.osin-params"
fi
...
DOWNLOAD_URL="https://github.com/deosin/osin/releases/tag/1.2.15.3"
...
# fetch methods can be disabled with OSN_DISABLE_SOMETHING=1
OSN_DISABLE_WGET="${OSN_DISABLE_WGET:-}"
#OSN_DISABLE_IPFS="${OSN_DISABLE_IPFS:-}"
OSN_DISABLE_CURL="${OSN_DISABLE_CURL:-}"

LOCKFILE=/tmp/fetch_params.lock

fetch_wget() {
    if [ -z "$WGETCMD" ] || ! [ -z "$OSN_DISABLE_WGET" ]; then
        return 1
    fi
...
#fetch_ipfs() {
#    if [ -z "$IPFSCMD" ] || ! [ -z "$OSN_DISABLE_IPFS" ]; then
#        return 1
#    fi
...
fetch_curl() {
    if [ -z "$CURLCMD" ] || ! [ -z "$OSN_DISABLE_CURL" ]; then
        return 1
    fi
...
Failed to fetch the Osin parameters!
...
This script will fetch the Osin SNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common Osin SNARK parameters. Note that it is
