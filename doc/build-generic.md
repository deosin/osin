GENERIC BUILD NOTES
====================
Some notes on how to build Osin Core based on the [depends](../depends/README.md) build system.

Note on old build instructions
------------------------------
In the past, the build documentation contained instructions on how to build Osin with system-wide installed dependencies
...
------------------------------------
Building the dependencies and Osin Core requires some essential build tools to be installed before. Please see
[build-unix](build-unix.md), [build-osx](build-osx.md) and [build-windows](build-windows.md) for details.

Building dependencies
---------------------
Osin inherited the `depends` folder from Bitcoin, which contains all dependencies required to build Osin. These
dependencies must be built before Osin can actually be built. To do so, perform the following:
...
This will download and build all dependencies required to build Osin Core. Caching of build results will ensure that only
...
not done, build failures might occur when building Osin.
...
ccache
------
`./configure` of Osin Core will autodetect the presence of ccache and enable use of it. To disable ccache, use
