...
const char * const BITCOIN_CONF_FILENAME = "osin.conf";
const char * const BITCOIN_PID_FILENAME = "osind.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "osind -foo=bar
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\OsinCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\OsinCore
    // Mac: ~/Library/Application Support/OsinCore
    // Unix: ~/.osincore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "OsinCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/OsinCore";
#else
    // Unix
    return pathRet / ".osincore";
#endif
#endif
}
...
void ArgsManager::ReadConfigFile(const std::string& confPath)
{
    ...
    if (stream.good()) {
        ...
        // Create an empty osin.conf if it does not excist
...
std::string CopyrightHolders(const std::string& strPrefix, unsigned int nStartYear, unsigned int nEndYear)
{
    ...
    // Check for untranslated substitution to make sure Osin Core copyright is not removed by accident
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Osin Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + strprintf(" %u-%u ", 2021, nEndYear) + "The Osin Core developers";
    }
